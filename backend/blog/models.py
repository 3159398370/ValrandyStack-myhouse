...
class Post(models.Model):
    ...
    featured_image = models.ImageField(_('特色图片'), upload_to='blog/featured/', blank=True, null=True)
    status = models.CharField(_('状态'), max_length=10, choices=STATUS_CHOICES, default='draft')
    author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='blog_posts', verbose_name=_('作者'))
...
class Comment(models.Model):
    ...
    评论模型
    """
    post = models.ForeignKey(Post, on_delete=models.CASCADE, related_name='comments', verbose_name=_('文章'))
    author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='blog_comments', verbose_name=_('作者'))
...
class Like(models.Model):
    ...
    点赞模型
    """
    post = models.ForeignKey(Post, on_delete=models.CASCADE, related_name='likes', verbose_name=_('文章'))
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='blog_likes', verbose_name=_('用户'))
    created_at = models.DateTimeField(_('创建时间'), auto_now_add=True)
